...
namespace Nxr.Internal
{
    ...
    public class HoloeverMarkerCamera : MonoBehaviour
    {
        public HoloeverMarker holoeverMarker;
        ...
        void Update()
        {
            if (holoeverService != null && holoeverService.IsMarkerRecognizeRunning && (holoeverMarker != null && holoeverMarker.IsVisible()) && updateProjMat < 10)
            {
                float[] array = holoeverService.GetMarkerProjectionMatrix();
                if (array != null)
                {
                    // Marker识别到才进行更新Projection
                    // Debug.Log(" >>>>>>>>>>>>>>>>>>>>>>>>>>cam.projectionMatrix : " + cam.projectionMatrix.ToString());
                    Matrix4x4 projectionMatrix = ARUtilityFunctions.MatrixFromFloatArray(holoeverService.GetMarkerProjectionMatrix());
            ...
            if (holoeverMarker != null && holoeverMarker.IsVisible())
            {
                Matrix4x4 cameraPose = holoeverMarker.CameraPose();
                ...
                Vector3 leftCameraPosDiff = holoeverMarker.CameraPosition(0) - arPosition;
                Vector3 rightCameraPosDiff = holoeverMarker.CameraPosition(1) - arPosition;
                ...
                //Debug.Log("DefaultPosition=" + arPosition.ToString("f4") + ", Left=" + holoeverMarker.CameraPosition(0).ToString("f4")
                //    + ", Right=" + holoeverMarker.CameraPosition(1).ToString("f4") + ", Left Diff=" + leftCameraPosDiff.ToString("f4")
        ...
        private void OnDestroy()
        {
            ...
            Debug.Log("HoloeverMarkerCamera.OnDestroy");
        }
