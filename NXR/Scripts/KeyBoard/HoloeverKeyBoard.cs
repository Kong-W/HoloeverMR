...
namespace Nxr.Internal
{
    ...
    public class HoloeverKeyBoard
    {

        public static HoloeverKeyBoard m_instance = null;

        public static HoloeverKeyBoard Instance
        {
            get
            {
                if (m_instance == null || m_instance.m_objSelf == null)
                    m_instance = new HoloeverKeyBoard();
                return m_instance;
            }
        }
        ...
        private HoloeverKeyBoardSingle[] m_arrCKeyBoardSingle = null;
        ...
        /// 初始字符串
        /// </summary>
        private string m_strInitChar = " Holoever KeyBoard ";

        private bool isShowing = false;

        public HoloeverKeyBoard()
        {
            // Debug.Log("------>Create HoloeverKeyBoard");
            m_objSelf = GameObject.Instantiate<GameObject>(Resources.Load<GameObject>("Keyboard/HoloeverKeyBoard"));
            m_objSelf.SetActive(false);
            keyBoardTransform = m_objSelf.transform;
            keyBoardTransform.position = new Vector3(1000, 1000, 1000);
            m_arrCKeyBoardSingle = keyBoardTransform.GetComponentsInChildren<HoloeverKeyBoardSingle>();
        ...
        public void Show(int _pageIndex, Vector3 position, Vector3 rotation)
        {
            if (isShowing)
            {
                Debug.Log("HoloeverKeyBoard is Showing.");
                return;
            }
            ...
            List<KeyBoardInfo> m_listKeyBoard = HoloeverCoreStaticDataManager.instance.GetKeyBoardInfoByPage(_pageIndex % 2);
        ...
        public void OnPressEnter(GameObject hit)
        {
            ...
            {
                HoloeverKeyBoardSingle _CKeyBoardSingle = hit.GetComponent<HoloeverKeyBoardSingle>();
